...
function useBroadcast(channel, onMsg) {
  const bcRef = useRef();
  useEffect(() => {
    // BroadcastChannel — works across same-origin tabs in same browser
    try { bcRef.current = new BroadcastChannel(channel); bcRef.current.onmessage = onMsg; }
    catch {}
    // localStorage storage event — fires in OTHER tabs when localStorage changes
    const handleStorage = (e) => {
      if (e.key && e.key.startsWith("smai_")) onMsg({ data: "sync" });
    };
  ...
  return () => {
    ...
    // Trigger storage event for other tabs by touching a sync key
    try { localStorage.setItem("smai_sync_" + channel, Date.now().toString()); } catch {}
  };
}
...
// ── Shared localStorage store — lets rooms persist across tabs & refreshes ──
const SS = {
  get: (key) => {
    try {
  ...
  set: (key, value) => {
    try {
      // Attach a timestamp so polling can detect changes across tabs
